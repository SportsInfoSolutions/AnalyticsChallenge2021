...
#Establish prior using student T and fit model.
```{r}
...
est <- mixedup::extract_random_effects(EPA_Mixed) %>%
  ...
  separate(id, c("Combo_ID", "Coverage_ID"), "_")
...
est  %>%
  ...
  labs(title = "Individual Expected EPA Per Route Combo Against Cover 3: Top 15",
       subtitle = "How each route combo increases CP after controlling for coverage type, and QB",
...
est  %>%
  ...
  ggplot(aes(x = reorder(CoverageScheme, value), value)) +
  geom_linerange(size = .5, aes(
    ymin = lower_2.5,
    ymax = upper_97.5
  ), color = "#838383") +
  ...
  theme(panel.background = element_rect(fill = "gray97"),
        axis.title.y=element_blank(),
        axis.ticks.y=element_blank())+
  labs(title = "Individual Probability of Completion for the Go - Seam Combo Against Each Coverage",
